...
cmdDocker proj command =
  do exec "ln -sf docker/Dockerfile.development Dockerfile"
     exec $ "docker build -t " ++ proj ++ "_devel ."
     exec "rm Dockerfile"
     void $ exec $ "docker run -w /" ++ proj ++ " -i -t -v $PWD/migrations:/" ++ proj ++ "/migrations -v $PWD/docker/data:/var/lib/postgresql -e CMD=\"" ++ command ++ "\" "
...
runDocker proj _ _ =
  do exec "ln -sf docker/Dockerfile.development Dockerfile"
     exec $ "docker build -t " ++ proj ++ "_devel ."
     exec "rm Dockerfile"
     void $ exec $ "docker run -w /" ++ proj ++ " -p 8000:8000 -i -t -v $PWD/docker/data:/var/lib/postgresql -v $PWD/migrations:/" ++ proj ++ "/migrations -v $PWD/snaplets:/" ++ proj ++ "/snaplets -v $PWD/templates:/" ++ proj ++ "/templates -v $PWD/emails:/" ++ proj ++ "/emails -v $PWD/static:/" ++ proj ++ "/static -v $PWD/src:/" ++ proj ++ "/src -v $PWD/devel.cfg:/" ++ proj ++ "/devel.cfg -v $PWD/defaults.cfg:/" ++ proj ++ "/defaults.cfg -e CMD=\"rivet run\" " ++ proj ++ "_devel /" ++ proj ++ "/run"
